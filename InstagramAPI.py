...
class InstagramAPI:
    ...
    def getRecentActivity(self):
        ...
        if self.LastJson['new_stories']:
            self.LastActivity = self.LastJson['new_stories'][0]['pk']
        else:
            self.LastActivity = self.LastJson['old_stories'][0]['pk']
        return activity
    ...
    def SendRequest(self, endpoint, post = None, login = False):
        ...
        self.s.headers.update ({'Connection' : 'close',
                                ...
                                'User-Agent' : self.USER_AGENT})
        if (post != None): # POST
            response = self.s.post(self.API_URL + endpoint, data=post, timeout=4) # , verify=False
        else: # GET
            response = self.s.get(self.API_URL + endpoint, timeout=4) # , verify=False
        ...
        if response.status_code == 200:
            ...
            if (response.status_code in [400, 403]):
                ...
                if self.error_400 >= 3:
                    # banned
                    print('Error 400/403... Sleeping...')
                    time.sleep((20 * 60 * 60) + (random.randint(0,60) * 60))
                    self.error_400 = 0
            # for debugging
            try:
                self.LastResponse = response
